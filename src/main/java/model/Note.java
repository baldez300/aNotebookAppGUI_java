...
/*
    ...
    When the user clicks the Add button, the application should save the note with its title and content. The saved notes should be displayed in a textual format, such as a separate section below the input elements.
    ...
    Implement the Controller, which serves as an intermediary between the View (GUI) and the Model. The Controller should have methods for handling user interactions and managing the notes. For example, when the user clicks the Add button, the Controller should create a new Note instance, populate it with the entered title and content, and add it to the Notebook.

    Create an FXML file using SceneBuilder to define the layout of the GUI. Set the appropriate event handlers for the user interactions, such as button clicks, in the FXML file.
    ...
    The application has the Model classes Note and Notebook with the relevan properties and methods (1 point)
